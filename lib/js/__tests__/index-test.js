...
test("opening the picker with only date picker", async () => {
  const picker = await openPicker({datePickerOnly: true});

  // Extra class should be added
  expect($('.c-datepicker').classList.contains('c-datepicker--date-only')).toEqual(true);

  // Check one element that should be gone
  setTimeout(() => {
    var elStyles = window.getComputedStyle($('.c-datepicker__header-date__time'));
    expect(elStyles.getPropertyValue('display')).toEqual('none');
  }, 1);
});

test("opening the picker with only time picker", async () => {
  const picker = await openPicker({timePickerOnly: true});

  // Extra class should be added
  expect($('.c-datepicker').classList.contains('c-datepicker--time-only')).toEqual(true);
  // Check one element that should be gone
  setTimeout(() => {
    var elStyles = window.getComputedStyle($('.c-datepicker__header-date__day'));
    expect(elStyles.getPropertyValue('display')).toEqual('none');
  }, 1);
});

test("opening the picker with time picker by default", async () => {
  const picker = await openPicker({timePickerDefault: true});

  // Check one element that should be gone
  setTimeout(() => {
    var elStyles = window.getComputedStyle($('.c-datepicker__clock'));
    expect(elStyles.getPropertyValue('display')).toEqual('none');
  }, 1);
});
