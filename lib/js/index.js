...
class DateTimePicker extends Events {
  constructor(options = {}) {
    ...
    const romeOptions = Object.assign(defaults().romeOptions, options.romeOptions);
    this.options = Object.assign(defaults(), options);
    this.options.styles = styles;
    this.options.romeOptions = romeOptions;
  ...
  initializeRome(container, validator) {
    ...
    const options = Object.assign(this.options.romeOptions, {
  ...
  onChangeDate(dateString) {
    const newValue = moment(this.value);
    const _dateString = dateString.split(' ')[0];
    const [year, month, date] = _dateString.split('-');
    newValue.set({ year: parseInt(year, 10), month: parseInt(month, 10) - 1, date: parseInt(date, 10) });
